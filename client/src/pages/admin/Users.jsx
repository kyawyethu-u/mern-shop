            ...
            <tbody>
                {
                    ...
                    <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap text-left">{user.name}</th>
                    <td className="px-6 py-4">{user.email}</td>
                    <td className="px-6 py-4">
                        {user.role === "admin" ? (<span className='text-green-600 font-medium'>{user.role}</span>) :
                         (<span className='text-blue-600'>{user.role}</span>)}
                    </td>
                    <td className="px-6 py-4">{ 
                        user.status === "active" &&
                        <span className="bg-green-400 text-sm p-1 rounded-md">{user.status}</span>}
                        { 
                        user.status === "banned" &&
                        <span className="bg-red-400 text-sm p-1 rounded-md">{user.status}</span>}
                     </td>
                    ...
                    <td className="px-6 py-4">
                    {
                        ...
                        <button type="button"
                         ...
                         onClick={()=>{banHandler(user._id)}}>Ban</button>:
                         <button type="button"
                         className="font-medium text-blue-600 hover:underline me-4"
                         onClick={()=>{unBanHandler(user._id)}}>Unban</button>
                    }
                    </td>
