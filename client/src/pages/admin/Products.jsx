            ...
            <tbody> 
                {
                    ...
                    <td className="px-6 py-4">
                        { product.status === "approve" ? <button type="button"
                         className="font-medium text-blue-600 hover:underline me-4"
                         onClick={()=>{rollbackHandler(product._id)}}>Roll Back</button> :
                         <button type="button"
                         className="font-medium text-blue-600 hover:underline me-4"
                         onClick={()=>{approveHandler(product._id)}}>Approve</button>
                         }
                         {product.status === "reject" ? <button type="button"
                         className="font-medium text-red-600 hover:underline me-4"
                         onClick={()=>{rollbackHandler(product._id)}}>Roll Back</button>:
                         <button type="button"
                         className="font-medium text-red-600 hover:underline me-4"
                         onClick={()=>{rejectHandler(product._id)}}>Reject</button>
                         }
                    </td>
                </tr>
                            ))
